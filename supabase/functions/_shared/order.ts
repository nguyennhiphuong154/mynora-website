...
export type SavedOrder = { id: string; request_code?: string; created_at: string; payload: OrderInput; items: { productId: string; productNameSnapshot: string; variantId?: string | null; variantNameSnapshot?: string | null; unitPriceSnapshot?: number | null; quantity: number }[] };
export function orderEmail(order: SavedOrder) {
  const p = order.payload;
  return { subject: `[MYNORA] Yêu cầu đặt bánh mới - ${p.customerName.replace(/[\r\n]/g, " ")}`,
    text: ["MYNORA · YÊU CẦU ĐẶT BÁNH", `Mã yêu cầu: ${order.request_code ?? `MYN-${order.id}`}`, "Trạng thái: Chờ MYNORA xác nhận", "", `Họ tên: ${p.customerName}`, `Điện thoại: ${p.phone}`, `Kênh liên hệ: ${{ phone: "Điện thoại", facebook: "Facebook", instagram: "Instagram" }[p.preferredContactChannel as "phone"]}`, "", "BÁNH KHÁCH ĐẶT", ...order.items.map(i => `${i.productNameSnapshot} × ${i.quantity}`), "", `Ngày nhận: ${p.requestedDate}`, `Khung giờ: ${p.requestedTimeSlot}`, `Nhận bánh: ${p.fulfillmentType === "delivery" ? "Giao hàng" : "Tự đến nhận"}`, ...(p.fulfillmentType === "delivery" ? [`Địa chỉ: ${p.deliveryAddress}`] : []), ...(p.differentRecipient ? [`Người nhận: ${p.recipientName}`, `Điện thoại người nhận: ${p.recipientPhone}`] : []), "", `Ghi chú: ${p.note || "Không có"}`, `Gửi lúc: ${order.created_at}`].join("\n") };
}
