...
export default function OperationsManager({ initialSettings }: { initialSettings: OperationsSettings }) {
  ...
  return <section className={styles.panel}><div className={styles.panelHeading}><div><p>VẬN HÀNH / SUPABASE</p><h2>Cấu hình công khai</h2></div></div><form className={styles.productForm} onSubmit={save}>
    ...
    <label><span>Email</span><input type="email" value={settings.contact.email} onChange={event => change("contact", "email", event.target.value)} required /></label>
