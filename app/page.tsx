...
export default async function Home() {
 const homeProducts = await getPublicCatalog();
  ...
  return <main className="home-page">
    ...
    <section className="home-order-journey" aria-labelledby="order-title"><div className="journey-heading"><p className="home-section-label">ĐẶT BÁNH CÙNG MYNORA</p><h2 id="order-title">Một hành trình<br />rất đơn giản.</h2></div><ol>{[["Chọn món bánh", "Khám phá menu và chọn món phù hợp."],["Gửi yêu cầu trước", `Gửi yêu cầu trước ít nhất ${order.minimumLeadTimeDays} ngày.`],["Nhận xác nhận", "MYNORA xác nhận ngày và khung giờ theo từng đơn."],["Bánh được chuẩn bị", "Bếp chuẩn bị bánh mới theo lịch đã hẹn."]].map(([title, text], index) => <li key={title}><span>0{index + 1}</span><h3>{title}</h3><p>{text}</p></li>)}</ol><Link className="journey-link" href="/huong-dan-dat-banh">Xem hướng dẫn đặt bánh <span aria-hidden="true">↗</span></Link></section>

    <section className="home-closing-cta" aria-labelledby="closing-title"><p className="home-section-label">MYNORA BAKERY</p><h2 id="closing-title">Bạn đã chọn được món bánh<br /><em>hôm nay chưa?</em></h2><div><Link className="closing-primary" href="/dat-banh">Đặt bánh cùng MYNORA <span aria-hidden="true">↗</span></Link><Link className="closing-secondary" href="/san-pham">Xem toàn bộ menu</Link></div></section>

    <footer className="home-footer"><div className="home-footer-brand"><img src="/images/brand/mynora-logo.png" alt="MYNORA Bakery" /><p>Bánh làm theo đơn, được chuẩn bị theo lịch.</p></div><div className="home-footer-links"><section><h2>Khám phá</h2><Link href="/san-pham">Sản phẩm</Link><Link href="/cau-chuyen-mynora">Câu chuyện MYNORA</Link><Link href="/nhat-ky-bep">Nhật ký bếp</Link></section><section><h2>Hỗ trợ</h2><Link href="/huong-dan-dat-banh">Cách đặt bánh</Link><Link href="/tra-cuu-don-hang">Tra cứu đơn</Link><Link href="/lien-he">Liên hệ</Link></section><section><h2>Chính sách</h2><Link href="/chinh-sach/dat-hang">Đặt hàng</Link><Link href="/chinh-sach/giao-hang">Giao hàng</Link><Link href="/chinh-sach/quyen-rieng-tu">Quyền riêng tư</Link></section></div><p className="home-copyright">© {new Date().getFullYear()} MYNORA. All rights reserved.</p></footer>
  </main>;
}
