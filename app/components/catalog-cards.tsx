...
import { orderStatusLabels, type CatalogProduct } from "../lib/site-data";
...
export function CatalogCards({ items }: { items: readonly CatalogProduct[] }) {
  return <div className="catalog-grid">{items.map((product, index) => <article className="catalog-card" key={product.slug}>
    ...
    <div className="catalog-card-copy"><p>{product.standardName}</p><h2><Link href={`/san-pham/${product.slug}`}>{product.displayName}</Link></h2><span className="product-coming-soon">{orderStatusLabels[product.orderStatus]}</span><Link className="catalog-card-link" href={`/san-pham/${product.slug}`}>Xem chi tiết <i aria-hidden="true">↗</i></Link></div>
  </article>)}</div>;
}
